...
func (b *ByteBuffer) ResetIt() *ByteBuffer {
	var m = b.mark
	if m < 0 {
...
func (b *ByteBuffer) ReadString(length int) (rs string, e error) {
	offset := b.off
	bs := b.buf[offset : offset+length]
	rs = string(bs)
	b.off += length
	return rs, nil
}
func (b *ByteBuffer) ReadUnsignedVarint() (value int, e error) {
	var i = 0
	var bt = 0
	var b1 = []byte{0}
	for {
		...
		if i > 28 {
			...
			return 0, errors.New(fmt.Sprintf("illegalVarintException(%d)", value))
		}
	}
	value |= bt << i
	return value, nil
}
func (b *ByteBuffer) ReadVarint() (value int, e error) {
	value, e = b.ReadUnsignedVarint()
	if e != nil {
		return 0, e
	}
	return int(uint32(value)>>1) ^ -(value & 1), nil
}
func (b *ByteBuffer) ReadVarLong() (value int64, e error) {
	var i = 0
	var bt int64
	var b1 = []byte{0}
	for {
		...
		i += 7
		if i > 63 {
			return 0, fmt.Errorf("illegalVarlongException(%d)", value)
		}

	}
	value |= bt << i
	return int64(uint64(value)>>1) ^ -(value & 1), e
}
func (b *ByteBuffer) Close() error {
	return nil
}

func (b *ByteBuffer) ReadToArray(dst *[]byte, offset int, length int) {
...
func (b *ByteBuffer) Trunc(offset int) *ByteBuffer {
	b.buf = b.buf[offset:]
	b.off = 0
	b.size = 0
	return b
}
func (b *ByteBuffer) Limit(size int) {
	b.size = size
}
func (b *ByteBuffer) Seek(offset int) {
	n, ok := b.tryGrowByReslice(offset)
	if !ok {
		b.grow(offset - n)
	}
	if b.size < offset {
...
func (b *ByteBuffer) Put(offset int, value byte) {
	b.buf[offset] = value
}
...
func (b *ByteBuffer) ResetByBuf(buf []byte) {
	b.buf = buf[:0]
	b.off = 0
	b.size = 0
	b.lastRead = opInvalid
}
func (b *ByteBuffer) ResetByBufForRead(buf []byte) {
	b.buf = buf
	b.off = 0
	b.size = 0
	b.lastRead = opInvalid
...
func (b *ByteBuffer) tryGrowByReslice(n int) (int, bool) {
	if i := b.off; /*len(b.buf)*/ n <= cap(b.buf)-i {
		// b.buf = b.buf[:l+n]
		if sub := len(b.buf) - i - n; sub < 0 {
			for i, Max := 0, -sub; i < Max; i++ {
...
func growSlice(b *[]byte, off, n int) {
	defer func() {
		if recover() != nil {
			panic(ErrTooLarge)
		}
	}()
	c := off + n // ensure enough space for n elements
	if twoB := 2 * cap(*b); c < twoB {
...
func growSlice_2(b []byte, off, n int) []byte {
	defer func() {
		if recover() != nil {
			panic(ErrTooLarge)
		}
