...
func TestDiskQueue(t *testing.T) {
	...
	dq := New(dqName, tmpDir, 1024, 4, 1<<10, 2500, 2*time.Second, t.Logf, t.Errorf)
	defer dq.Close()
	NotNil(t, dq)
...
func TestDiskQueueRoll(t *testing.T) {
	...
	ml := int64(len(msg))
	dq := New(dqName, tmpDir, 10*(ml+4), int32(ml), 1<<10, 2500, 2*time.Second, t.Logf, t.Errorf)
	defer dq.Close()
	NotNil(t, dq)
	Equal(t, int64(0), dq.Depth())
...
func TestDiskQueueEmpty(t *testing.T) {
	...
	defer os.RemoveAll(tmpDir)
	msg := bytes.Repeat([]byte{0}, 10)
	dq := New(dqName, tmpDir, 100, 0, 1<<10, 2500, 2*time.Second, t.Logf, t.Errorf)
	defer dq.Close()
	NotNil(t, dq)
	Equal(t, int64(0), dq.Depth())
...
func TestDiskQueueCorruption(t *testing.T) {
	...
	dq := New(dqName, tmpDir, 1000, 10, 1<<10, 5, 2*time.Second, t.Logf, t.Errorf)
	defer dq.Close()
...
func TestDiskQueueSyncAfterRead(t *testing.T) {
	...
	defer os.RemoveAll(tmpDir)
	dq := New(dqName, tmpDir, 1<<11, 0, 1<<10, 2500, 50*time.Millisecond, t.Logf, t.Errorf)
	defer dq.Close()
...
func TestDiskQueueTorture(t *testing.T) {
	...
	defer os.RemoveAll(tmpDir)
	dq := New(dqName, tmpDir, 262144, 0, 1<<10, 2500, 2*time.Second, t.Logf, t.Errorf)
	NotNil(t, dq)
	Equal(t, int64(0), dq.Depth())
...
func TestDiskQueueResize(t *testing.T) {
	...
	ml := int64(len(msg))
	dq := New(dqName, tmpDir, 8*(ml+4), int32(ml), 1<<10, 2500, time.Second, t.Logf, t.Errorf)
	NotNil(t, dq)
	Equal(t, int64(0), dq.Depth())
	...
	dq.Close()
	dq = New(dqName, tmpDir, 10*(ml+4), int32(ml), 1<<10, 2500, time.Second, t.Logf, t.Errorf)
...
func benchmarkDiskQueuePut(size int64, b *testing.B) {
	...
	defer os.RemoveAll(tmpDir)
	dq := New(dqName, tmpDir, 1024768*100, 0, 1<<20, 2500, 2*time.Second, b.Logf, b.Errorf)
	defer dq.Close()
	b.SetBytes(size)
	data := make([]byte, size)
	b.StartTimer()
...
func benchmarkDiskQueueGet(size int64, b *testing.B) {
	...
	defer os.RemoveAll(tmpDir)
	dq := New(dqName, tmpDir, 1024768, 0, 1<<30, 2500, 2*time.Second, b.Logf, b.Errorf)
	defer dq.Close()
	b.SetBytes(size)
	data := make([]byte, size)
	for i := 0; i < b.N; i++ {
